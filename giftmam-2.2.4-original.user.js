        ...
        async function runBatch(forceStart = false) {
            ...
            while (virtualQueue.length > 0 && processed < maxGifts) {
                ...
                if (result.success || (result.error && result.error.includes("daily cap"))) {
                    ...
                    stopRequested = true;
                    isSoftPaused = true;
                    window.log && window.log(`⏳ Rate Limit hit. Soft pausing for 60 seconds...`, 'warn');
                    // We intentionally leave the user in the queue here so it retries upon resume
                    // eslint-disable-next-line no-loop-func
                    setTimeout(() => {
                        if (isAutoActive && isSoftPaused && !isRunning) {
                            window.log && window.log(`🔄 Attempting auto-resume...`, 'info');
                            isSoftPaused = false;
                            runBatch(true);
                        }
                    }, 60 * 1000);
                    ...
                    stopRequested = true;
                    isSoftPaused = true;
                    window.log && window.log(`⏳ Server error. Soft pausing for 5 minutes...`, 'warn');
                    // We intentionally leave the user in the queue here so it retries upon resume
                    // eslint-disable-next-line no-loop-func
                    setTimeout(() => {
                        if (isAutoActive && isSoftPaused && !isRunning) {
                ...
                if (!stopRequested && virtualQueue.length > 0) {
                    ...
                    if (isHighAmt) {
                        // 3-3.5 second delay to balance the delay injected inside sendGift
                        await new Promise(r => setTimeout(r, 3000 + Math.random() * 500));
                    } else {
                        // 5-7 second delay; rolling window array handles the hard limits
